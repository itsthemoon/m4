...
const store = function(config) {
  ...
  const del = (key, callback, gid = context.gid) => {
    if (!key) {
      key = getID(value);
    }
  ...
  const reconf = (previousState, callback, gid = context.gid) => {
    ...
    fetchNodeIDs((error, currentNodeIDs) => {
      ...
      let previousNodeIDs = Object.keys(previousState);
      let keysToRelocate = [];
        ...
        keysToRelocate.forEach((key) => {
          get(key, (getError, value) => {
            ...
            del(key, (delError, value) => {
              ...
              const newTargetNode = context.hash(getID(key), currentNodeIDs);
              const remote = {
